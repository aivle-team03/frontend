...
function getAreaLatestInspectionHistories(record) {
  ...
  return record.areas.map((area, index) => {
    ...
    return {
      ...
      status: INSPECTION_HISTORY_STATUSES[(index + recordIdLength) % INSPECTION_HISTORY_STATUSES.length],
    }
  })
}
...
function InspectionListPage() {
  ...
  const selectedAreaHistories = useMemo(
    () => (selectedRecord ? getAreaLatestInspectionHistories(selectedRecord) : []),
    [selectedRecord]
  )
            ...
            <div className="inspection-detail-body">
              ...
              <div className="inspection-detail-section">
                ...
                <div className="area-history-list">
                  {selectedAreaHistories.map((history) => (
                    <div className="area-history-item" key={history.area}>
                      <strong>{history.area}</strong>
                      <span>{history.dateTime}</span>
                      <span>{history.manager}</span>
                      <b className={history.status === '조치 필요' ? 'needs-action' : ''}>
                        {history.status}
                      </b>
                    </div>
                  ))}
