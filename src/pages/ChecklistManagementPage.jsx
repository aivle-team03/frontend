...
import { translateUi, useUiLanguage } from '../utils/uiLanguage.js'
...
function ChecklistManagementPage() {
    ...
    const loadRecords = async () => {
      ...
      try {
        ...
        const actionRecords = actions.map((item) => normalizeRecord({
          ...
          actionType: item.type || item.source_type || '-',
  ...
  return <section className={`checklist-management-page${isRecordsLoading ? ' is-data-loading' : ''}`} aria-busy={isRecordsLoading}>
    <article className="management-table-card"><div className="management-table-header"><div><span className="section-kicker">CHECKLIST OVERVIEW</span><h3>{t('담당자 배정')}</h3><p>{t('일시를 확인하고 점검·조치 담당자를 배정합니다.')}</p></div><div className="management-header-actions"><button className="checklist-create-button" type="button" onClick={() => setIsCreateOpen(true)}><AddRoundedIcon /> {t('항목 추가')}</button><div className="assignment-type-toggle" role="tablist" aria-label={t('항목 유형')}><button className={recordTypeFilter === 'inspection' ? 'is-active' : ''} type="button" role="tab" aria-selected={recordTypeFilter === 'inspection'} onClick={() => changeRecordType('inspection')}>{t('점검')}</button><button className={recordTypeFilter === 'action' ? 'is-active' : ''} type="button" role="tab" aria-selected={recordTypeFilter === 'action'} onClick={() => changeRecordType('action')}>{t('조치')}</button></div></div></div>
      ...
      <div className="bulk-assign-toolbar"><span>{t('선택')} <strong>{selected.length}</strong>{t('건')}</span><div>{recordTypeFilter === 'inspection' ? <button type="button" disabled={!chosen.length} onClick={() => setAssignmentMode('inspection')}><AssignmentIndOutlinedIcon /> {t('점검 담당자 배정')}</button> : <button type="button" disabled={!actionEnabled} onClick={() => setAssignmentMode('action')}><AssignmentIndOutlinedIcon /> {t('조치 담당자 배정')}</button>}</div></div>
      <div className="checklist-table-wrap"><table className="checklist-management-table master-checklist-table"><thead><tr><th className="checklist-select-col"><input type="checkbox" checked={visible.length > 0 && visible.every((item) => selected.includes(item.id))} onChange={(event) => setSelected((current) => event.target.checked ? [...new Set([...current, ...visible.map((item) => item.id)])] : current.filter((id) => !visible.some((item) => item.id === id)))} /></th><th>{t('점검 이름')}</th><th>{t('적용 구역')}</th><th>{t(recordTypeFilter === 'action' ? '타입' : '점검 담당자')}</th>{recordTypeFilter === 'action' && <th>{t('조치 담당자')}</th>}{recordTypeFilter === 'inspection' && <th>{t('일시')}</th>}<th>{t('진행 상태')}</th></tr></thead><tbody>{isRecordsLoading ? <ChecklistTableSkeletonRows /> : visible.map((item) => <tr className="checklist-detail-row" key={item.id} onClick={() => openDetail(item)}><td className="checklist-select-col"><input type="checkbox" checked={selected.includes(item.id)} onClick={(event) => event.stopPropagation()} onChange={() => toggle(item.id)} /></td><td><strong>{item.name}</strong><span className="table-category">{t(item.category)}</span></td><td title={item.location}><span className="location-cell">{formatLocationSummary(item.location)}</span></td><td>{recordTypeFilter === 'action' ? <span className="table-category">{t(item.actionType || '-')}</span> : <Assignee value={item.inspectionAssignee} />}</td>{recordTypeFilter === 'action' && <td><Assignee value={item.actionAssignee} /></td>}{recordTypeFilter === 'inspection' && <td>{item.dateTime}</td>}<td><Status value={t(item.progress)} /></td></tr>)}</tbody></table></div>
      <footer className="checklist-pagination"><span>{t('총')} <strong>{filtered.length}</strong>{t('건')}</span><div><button type="button" disabled={active === 0} onClick={() => setPage((current) => current - 1)}><ChevronLeftRoundedIcon /></button><b>{active + 1} / {pageCount}</b><button type="button" disabled={active === pageCount - 1} onClick={() => setPage((current) => current + 1)}><ChevronRightRoundedIcon /></button></div></footer></article>{detailItem && <ChecklistDetailModal item={detailItem} onCycleChange={updateCycle} onClose={() => setDetailItem(null)} />}{isCreateOpen && <CreateModal initialType={recordTypeFilter} onClose={() => setIsCreateOpen(false)} onCreate={addItem} />}{assignmentMode && <AssignmentModal mode={assignmentMode} count={selected.length} members={members} query={memberQuery} onQueryChange={setMemberQuery} onAssign={assignMember} onClose={() => { setMemberQuery(''); setAssignmentMode(null) }} />}</section>
}
...
function Assignee({ value }) { if (!value) return <span className="no-photo">{translateUi('미배정')}</span>; if (value === '게시판') return <span className="assignee-cell is-board-source"><i>{translateUi('게시판')}</i></span>; return <span className="assignee-cell"><i>{value[0]}</i>{value}</span> }
...
function ChecklistDetailModal({ item, onCycleChange, onClose }) {
  ...
  return <div className="assignment-modal-backdrop" onMouseDown={onClose}><section className={`checklist-detail-modal master-detail-modal ${isInspection ? 'is-inspection-detail' : 'is-action-detail'}`} onMouseDown={(event) => event.stopPropagation()}><header><div><span>CHECKLIST DETAIL</span><h3>{item.name}</h3><p>{isInspection ? `${item.category} · ${item.cycle} 점검` : `${item.category} · 조치 항목`}</p></div><button type="button" aria-label="닫기" onClick={onClose}>×</button></header><div className="master-detail-body"><section className="master-summary-grid">{isInspection && <><div><span>점검 주기</span><strong>{inspectionCycle}</strong></div><div><span>일시</span><strong>{item.dateTime}</strong></div></>}<div><span>{isInspection ? '점검 담당자' : '조치 담당자'}</span><strong>{isInspection ? item.inspectionAssignee || '미배정' : item.actionAssignee || '미배정'}</strong></div><div><span>진행 상태</span><Status value={item.progress} /></div></section><section className="master-info-grid"><div className="is-wide"><span>내용</span><p>{inspectionContent}</p></div></section><section><div className="master-section-heading"><h4>적용 구역</h4><span>{locations.length}곳</span></div><div className="master-location-list">{locations.map((location) => <span key={location}>{location}</span>)}</div></section><section><div className="master-section-heading"><h4>점검 이력</h4><span>최근 {inspectionHistory.length}건</span></div><div className="master-history-list">{inspectionHistory.map((entry, index) => <div key={`${entry.date}-${index}`}><span>{entry.date}</span><strong>{entry.location}</strong><span>{entry.manager}</span><Status value={entry.status} /></div>)}</div></section><section><div className="master-section-heading"><h4>조치 이력</h4><span>{actionHistory.length}건</span></div>{actionHistory.length ? <div className="master-history-list">{actionHistory.map((entry) => <div key={entry.date}><span>{entry.date}</span><strong>{entry.manager}</strong><span>조치 담당자</span><Status value={entry.status} /></div>)}</div> : <div className="checklist-empty">등록된 조치 이력이 없습니다.</div>}</section></div><footer><span>점검 및 조치 진행 내역을 확인합니다.</span><button type="button" onClick={onClose}>닫기</button></footer></section></div>
}
...
function CreateModal({ initialType, onClose, onCreate }) {
  ...
  const submit = async (event) => {
    ...
    if (isInspection) {
      try {
        ...
        const targetLocations = locations.length > 0 ? locations : ['구역 미지정']

        await Promise.all(
          targetLocations.map((loc) =>
            axios.post(
              `${API_BASE_URL}/api/inspection/histories/create`,
              {
                name: form.name.trim(),
                date: form.dateTime,
                location: loc,
  ...
  return <div className="assignment-modal-backdrop" onMouseDown={onClose}><section className="assignment-modal checklist-create-modal" onMouseDown={(event) => event.stopPropagation()}><header><div><span>ITEM CREATE</span><h3>{t(isInspection ? '점검 항목 추가' : '조치 항목 추가')}</h3><p>{t('전체 체크리스트에 새 항목을 등록합니다.')}</p></div><button type="button" onClick={onClose} disabled={isSubmitting}>×</button></header><form className={`checklist-create-form${isInspection ? '' : ' is-action-create'}`} onSubmit={submit}><section className="action-risk-picker is-wide"><span>{t('위험요인')}<b className="required-mark">*</b></span><div>{visibleRiskCategories.map((item) => <button className={Number(selectedEventCategoryId) === Number(item.category_id) ? 'is-selected' : ''} type="button" key={item.category_id} onClick={() => setSelectedEventCategoryId(item.category_id)} disabled={isSubmitting}><span>{t(item.category)}</span><strong>{t(item.category_name)}</strong><em>{t(item.risk_level)}</em></button>)}{!riskCategories.length && <p>{t('위험요인 목록이 없습니다.')}</p>}</div>{riskCategories.length > riskPageSize && <nav className="action-risk-pagination" aria-label={t('위험요인 페이지')}><button type="button" onClick={() => setRiskPage((page) => Math.max(1, page - 1))} disabled={isSubmitting || riskPage === 1}>{t('이전')}</button><strong>{riskPage} / {riskTotalPages}</strong><button type="button" onClick={() => setRiskPage((page) => Math.min(riskTotalPages, page + 1))} disabled={isSubmitting || riskPage === riskTotalPages}>{t('다음')}</button></nav>}</section><label className="is-wide"><span>{t(isInspection ? '점검 이름' : '조치 이름')}<b className="required-mark">*</b></span><input value={form.name} onChange={(event) => update('name', event.target.value)} placeholder={t(isInspection ? '예: 비상구 피난 통로 점검' : '예: 소화기 압력 게이지 교체')} disabled={isSubmitting} /></label><label className="is-wide"><span>{t(isInspection ? '점검 내용' : '조치 내용')}<b className="required-mark">*</b></span><textarea value={form.content} onChange={(event) => update('content', event.target.value)} placeholder={t(isInspection ? '점검 시 확인할 기준이나 내용을 입력하세요' : '조치 시 확인할 기준이나 내용을 입력하세요')} rows="4" disabled={isSubmitting} /></label><label><span>{t('분류')}</span><select value={form.category} onChange={(event) => update('category', event.target.value)} disabled={isSubmitting}>{CATEGORY.map((category) => <option key={category} value={category}>{t(category)}</option>)}</select></label>{isInspection && <label><span>{t('점검 주기')}</span><select value={form.cycle} onChange={(event) => update('cycle', event.target.value)} disabled={isSubmitting}><option>{t('매일')}</option><option>{t('매주')}</option><option>{t('매월')}</option></select></label>}<label className="is-wide"><span>{t('적용 구역')}<b className="required-mark">*</b></span><input value={form.location} onChange={(event) => update('location', event.target.value)} placeholder={t('여러 구역은 쉼표(,)로 구분하세요')} disabled={isSubmitting} /></label>{isInspection && <label><span>{t('일시')}</span><input type="datetime-local" value={form.dateTime} onChange={(event) => update('dateTime', event.target.value)} disabled={isSubmitting} /></label>}<footer><span>{t(isSubmitting ? '등록 중입니다.' : (isInspection ? '점검 대기 상태로 등록됩니다.' : '조치 대기 상태로 등록됩니다.'))}</span><div><button type="button" onClick={onClose} disabled={isSubmitting}>{t('취소')}</button><button type="submit" disabled={isSubmitting}>{t(isSubmitting ? '등록 중...' : '등록')}</button></div></footer></form></section></div>
}
