...
function ChecklistManagementPage() {
  ...
  const addItem = async (item) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/api/checklists`, {
        content: item.name,
        camera_id: item.cameraId,
        type: item.type === 'inspection' ? '점검' : '조치',
      })
      const savedItem = { ...item, id: `database-${response.data.checklist_id}`, databaseId: response.data.checklist_id }
      setRecords((current) => { const next = [savedItem, ...current]; saveChecklistManagementRecords(next); return next })
      setIsCreateOpen(false)
      setPage(0)
    } catch (error) {
      console.error('체크리스트 DB 저장 실패:', error)
      alert('체크리스트를 DB에 저장하지 못했습니다. CCTV 선택과 서버 상태를 확인해 주세요.')
    }
  }
  const updateCycle = (id, cycle) => {
    setRecords((current) => {
      const next = current.map((item) => item.id === id ? { ...item, cycle } : item)
      saveChecklistManagementRecords(next)
      return next
    })
    setDetailItem((current) => current?.id === id ? { ...current, cycle } : current)
  }
...
function ChecklistDetailModal({ item, onCycleChange, onClose }) {
  ...
  const inspectionHistory = [{ date:item.dateTime, location:item.location, manager:item.inspectionAssignee || '미배정', status:item.progress.startsWith('점검') ? item.progress : '점검 완료' },{ date:'2026-07-24 09:10', location:item.location, manager:item.inspectionAssignee || '미배정', status:'점검 완료' }]
...
function CreateModal({ cctvs, onClose, onCreate }) { const [type, setType] = useState('inspection'); const [form, setForm] = useState({ name:'', location:'', category:'시설 안전', cycle:'매일', dateTime:'2026-07-25T09:00', cameraId:'' }); const update = (key, value) => setForm((current) => ({ ...current, [key]: value })); const submit = (event) => { event.preventDefault(); if (!form.name.trim() || !form.location.trim() || !form.cameraId) return; onCreate({ ...form, cameraId:Number(form.cameraId), type, cycle:type === 'inspection' ? form.cycle : null, name:form.name.trim(), location:form.location.trim(), inspectionAssignee:'', actionAssignee:'', progress:type === 'inspection' ? '점검 대기' : '조치 대기' }) }; return <div className="assignment-modal-backdrop" onMouseDown={onClose}><section className="assignment-modal checklist-create-modal" onMouseDown={(event) => event.stopPropagation()}><header><div><span>ITEM CREATE</span><h3>{type === 'inspection' ? '점검 항목 추가' : '조치 항목 추가'}</h3><p>전체 체크리스트에 새 항목을 등록합니다.</p></div><button type="button" onClick={onClose}>×</button></header><form className="checklist-create-form" onSubmit={submit}><div className="item-type-toggle"><button className={type === 'inspection' ? 'is-active' : ''} type="button" onClick={() => setType('inspection')}>점검 항목</button><button className={type === 'action' ? 'is-active' : ''} type="button" onClick={() => setType('action')}>조치 항목</button></div><label className="is-wide"><span>{type === 'inspection' ? '점검 이름' : '조치 이름'}</span><input value={form.name} onChange={(event) => update('name', event.target.value)} placeholder={type === 'inspection' ? '예: 비상구 피난 통로 점검' : '예: 소화기 압력 게이지 교체'} /></label><label><span>CCTV</span><select required value={form.cameraId} onChange={(event) => update('cameraId', event.target.value)}><option value="">CCTV 선택</option>{cctvs.map((cctv) => <option key={cctv.cctv_id} value={cctv.cctv_id}>{cctv.cctv_name} · {cctv.location}</option>)}</select></label><label><span>분류</span><select value={form.category} onChange={(event) => update('category', event.target.value)}><option>시설 안전</option><option>화재 예방</option><option>작업 안전</option></select></label>{type === 'inspection' && <label><span>점검 주기</span><select value={form.cycle} onChange={(event) => update('cycle', event.target.value)}><option>매일</option><option>매주</option><option>매월</option></select></label>}<label className="is-wide"><span>적용 구역</span><input value={form.location} onChange={(event) => update('location', event.target.value)} placeholder="여러 구역은 쉼표(,)로 구분하세요" /></label><label><span>일시</span><input type="datetime-local" value={form.dateTime} onChange={(event) => update('dateTime', event.target.value)} /></label><footer><span>{type === 'inspection' ? '점검 대기 상태로 등록됩니다.' : '조치 대기 상태로 등록됩니다.'}</span><div><button type="button" onClick={onClose}>취소</button><button type="submit">등록</button></div></footer></form></section></div> }
