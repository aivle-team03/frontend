...
function ChecklistManagementPage() {
  ...
  const [filters, setFilters] = useState({ location: '전체 구역', category: '전체 카테고리', assignment: '전체 배정', query: '' })
  ...
  const filtered = useMemo(() => records.filter((item) => {
    ...
    return item.date.startsWith(selectedMonth) && (filters.location === '전체 구역' || item.location.includes(filters.location)) && (filters.category === '전체 카테고리' || item.category === filters.category) && (filters.assignment === '전체 배정' || item.assignment === filters.assignment) && (!query || item.name.includes(query) || item.assignee.includes(query) || item.completedBy.includes(query))
  ...
  const reset = () => { setFilters({ location: '전체 구역', category: '전체 카테고리', assignment: '전체 배정', query: '' }); setPage(0) }
  ...
  return <section className="checklist-management-page">
    ...
    <article className="management-table-card"><div className="management-table-header"><div><span className="section-kicker"><FilterAltOutlinedIcon /> CHECKLIST OVERVIEW</span><h3>전체 체크리스트</h3></div><label className="management-date"><CalendarTodayOutlinedIcon /><input type="month" value={selectedMonth} onChange={(event) => { setSelectedMonth(event.target.value); setPage(0) }} /><span>{monthRange}</span></label></div><div className="management-filters"><FilterSelect value={filters.location} options={['전체 구역', 'A동', 'B동', 'C동']} onChange={(value) => updateFilter('location', value)} /><FilterSelect value={filters.category} options={['전체 카테고리', '시설 안전', '소방 안전', '작업 안전', '전기 안전']} onChange={(value) => updateFilter('category', value)} /><FilterSelect value={filters.assignment} options={['전체 배정', '배정 완료', '미배정']} onChange={(value) => updateFilter('assignment', value)} /><label className="management-search"><SearchRoundedIcon /><input value={filters.query} onChange={(event) => updateFilter('query', event.target.value)} placeholder="체크리스트명 또는 담당자 검색" /></label><button className="filter-reset" type="button" onClick={reset}><RestartAltRoundedIcon />초기화</button></div><div className="checklist-table-wrap"><table className="checklist-management-table"><thead><tr><th>체크리스트명</th><th>구역/위치</th><th>대상자</th><th>일시</th><th>배정 상태</th><th>담당자</th><th>증빙</th><th>작업</th></tr></thead><tbody>{visibleRecords.map((item) => <tr className="checklist-detail-row" key={item.id} onClick={() => setSelected({ ...item, mode: 'detail' })}><td><strong>{item.name}</strong><span className="table-category">{item.category}</span></td><td><span className="location-cell"><LocationOnOutlinedIcon />{item.location}</span></td><td><span className="assignee-cell performer-cell"><i>{item.completedBy.slice(0, 1)}</i>{item.completedBy}</span></td><td>{item.date}</td><td><StatusBadge value={item.assignment} /></td><td><span className="assignee-cell"><i>{item.assignee.slice(0, 1)}</i>{item.assignee}</span></td><td>{item.photo ? <span className="photo-chip"><ImageOutlinedIcon />첨부</span> : <span className="no-photo">-</span>}</td><td>{item.assignment === '배정 완료' ? <button className="change-button" type="button" onClick={(event) => { event.stopPropagation(); setSelected({ ...item, mode: 'assign' }); setMemberQuery('') }}><SwapHorizRoundedIcon />담당자 변경</button> : <button className="assign-button" type="button" onClick={(event) => { event.stopPropagation(); setSelected({ ...item, mode: 'assign' }); setMemberQuery('') }}><AssignmentIndOutlinedIcon />담당자 배정</button>}</td></tr>)}</tbody></table>{!filtered.length && <div className="checklist-empty">조건에 맞는 체크리스트가 없습니다.</div>}</div><footer className="checklist-pagination"><span>총 <strong>{filtered.length}</strong>건</span><div><button type="button" aria-label="이전 페이지" disabled={activePage === 0} onClick={() => setPage((current) => current - 1)}><ChevronLeftRoundedIcon /></button><b>{activePage + 1} / {pageCount}</b><button type="button" aria-label="다음 페이지" disabled={activePage === pageCount - 1} onClick={() => setPage((current) => current + 1)}><ChevronRightRoundedIcon /></button></div></footer></article>
...
function AssignmentModal({ checklist, members, query, onQueryChange, onClose, onAssign }) {
  const isChange = checklist.assignment === '배정 완료'
  const actionLabel = isChange ? '담당자 변경' : '담당자 배정'

  return <div className="assignment-modal-backdrop" role="presentation" onMouseDown={onClose}><section className="assignment-modal" role="dialog" aria-modal="true" aria-labelledby="assignment-title" onMouseDown={(event) => event.stopPropagation()}><header><div><span>CHECKLIST ASSIGNMENT</span><h3 id="assignment-title">{actionLabel}</h3><p>{isChange ? '현재 담당자를 변경할 현장관리자를 선택해 주세요.' : '점검 내용에 적합한 현장관리자를 선택해 주세요.'}</p></div><button type="button" aria-label={`${actionLabel} 창 닫기`} onClick={onClose}><CloseRoundedIcon /></button></header><div className="assignment-checklist-summary"><div className="summary-icon"><WarningAmberRoundedIcon /></div><div><span>점검 항목</span><strong>{checklist.name}</strong><p><LocationOnOutlinedIcon />{checklist.location} <b>·</b> {checklist.date}</p></div><div className={`risk-level risk-${checklist.level}`}><small>위험도</small><strong>{checklist.level}</strong></div></div><div className="assignment-member-section"><div className="assignment-section-heading"><div><span>MANAGER LIST</span><h4>{isChange ? '변경할 담당자 선택' : '담당자 선택'}</h4></div><label><SearchRoundedIcon /><input value={query} onChange={(event) => onQueryChange(event.target.value)} placeholder="이름 또는 담당 구역 검색" /></label></div><div className="assignment-member-list">{members.map((member) => <button type="button" className={`assignment-member${!member.available ? ' is-unavailable' : ''}`} key={member.name} disabled={!member.available} onClick={() => onAssign(member)}><span className="member-avatar">{member.name.slice(0, 1)}</span><span className="member-copy"><strong>{member.name}</strong><small>{member.zone} 담당 · 현재 업무 {member.workload}건</small></span><span className={`member-availability${member.available ? ' is-available' : ''}`}>{member.available ? '배정 가능' : '업무 집중'}</span><PersonOutlineRoundedIcon className="member-arrow" /></button>)}{!members.length && <p className="member-empty">검색 결과가 없습니다.</p>}</div></div><footer><span>{isChange ? '담당자 선택 시 즉시 변경되며, 현장관리자에게 알림이 전송됩니다.' : '담당자 선택 시 즉시 배정되며, 현장관리자에게 알림이 전송됩니다.'}</span><button type="button" onClick={onClose}>닫기</button></footer></section></div>
}
